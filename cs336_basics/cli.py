...
def main():
    ...
    parser.add_argument(
        ...
        default = config.get_data_dir(1) + "verysmall_tiny.txt"
    )
    parser.add_argument(
        "--vocab_size",
        help = "Size of the finished vocabulary",
        type = int,
        default = 300
    )
    parser.add_argument(
        "--special_tokens",
        help = "List of special token strings",
        type = list[bytes],
        default = ['<|endoftext|>']
    )
    args = parser.parse_args()

    bpe_tokenizer_trainer.train_tokenizer(
        args.input_path,
        args.vocab_size,
        args.special_tokens
    )
